...
type ARCCache[K comparable, V comparable] struct {
...
func NewARC[K comparable, V comparable](size int) (*ARCCache[K, V], error) {
	// Create the sub LRUs
	b1, err := simplelru.New[K, V](size, nil)
	if err != nil {
		return nil, err
	}
	b2, err := simplelru.New[K, V](size, nil)
	if err != nil {
		return nil, err
	}
	t1, err := simplelru.New[K, V](size, nil)
	if err != nil {
		return nil, err
	}
	t2, err := simplelru.New[K, V](size, nil)
	if err != nil {
		return nil, err
	}
